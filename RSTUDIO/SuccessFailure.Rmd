...
# Add total failures and total successes
session_data$Total_Successes <- sapply(data_list, function(df) sum(df$Success, na.rm = TRUE))
session_data$Total_Failures <- sapply(data_list, function(df) sum(df$Failure, na.rm = TRUE))
...
# Make a table with six rows one for each session and then hours, total failures and total successes
...
# Filter for only the date 2025-05-13
all_data_2025_05_13 <- all_data %>%
  filter(as.Date(Timestamp) == as.Date("2025-05-13"))

# Summarize data into success and failures
successes <- c(
  sum(all_data_2025_05_13$Success[all_data_2025_05_13$AI_Initiates == TRUE]), 
  sum(all_data_2025_05_13$Success[all_data_2025_05_13$AI_Initiates == FALSE])
  )

failures <- c(
  sum(all_data_2025_05_13$Failure[all_data_2025_05_13$AI_Initiates == TRUE]), 
  sum(all_data_2025_05_13$Failure[all_data_2025_05_13$AI_Initiates == FALSE])
  )
...
p <- ggplot(results_df, aes(x = Group, y = Success_Rate)) +
  ...
  theme_minimal()
p
ggsave("plots/Success_Rate_by_AI_Initiation_Setup4.png", plot = p, width = 8, height = 6, dpi = 300, bg = "white")
