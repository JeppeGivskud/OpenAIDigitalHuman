...
successes <- c(sum(all_data$Success[all_data$AI_Initiates == TRUE]), sum(all_data$Success[all_data$AI_Initiates == FALSE]), sum(all_data$Success[all_data$AI_Initiates == FALSE & all_data$Asks_for_Help == FALSE]))

failures <- c(sum(all_data$Failure[all_data$AI_Initiates == TRUE]), sum(all_data$Failure[all_data$AI_Initiates == FALSE ]),sum(all_data$Failure[all_data$AI_Initiates == FALSE &all_data$Asks_for_Help == FALSE]))
...
noASK <- analyze(successes[3], failures[3])
...
results_df <- data.frame(
  Group = c("AI Initiated", "User Initiated", "User Initiated without asked for help"),
  Successes = successes,
  Failures = failures,
  Success_Rate = c(ai_result$success_rate, user_result$success_rate, noASK$success_rate),
  SD = c(ai_result$sd, user_result$sd, noASK$sd),
  CI_Low = c(ai_result$ci_95[1], user_result$ci_95[1], noASK$ci_95[1]),
  CI_High = c(ai_result$ci_95[2], user_result$ci_95[2], noASK$ci_95[2])
)
# Print the results
print(results_df)
...
p <- ggplot(results_df, aes(x = Group, y = Success_Rate)) +
  ...
  theme_minimal()
p
ggsave("plots/Success Rate by AI Initiation asking is failing.png", plot = p, width = 8, height = 3, dpi = 300, bg = "white")
