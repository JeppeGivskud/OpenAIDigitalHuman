...
Questionnaire_plot <- function(data, questions, columns, title, title_offset) {
  ...
  # 1. Extract the relevant questionnaire items
  questionnaire_items <- data[columns]

    # 2. Compute Cronbach's alpha with auto key check
  overall_alpha <- psych::alpha(questionnaire_items, check.keys = FALSE)
  cor(questionnaire_items, use = "pairwise.complete.obs")  # Compute correlation matrix
  
  # Correlation matrix
  cor_matrix <- cor(questionnaire_items, use = "pairwise.complete.obs")
  cor_matrix_rounded <- round(cor_matrix, 2)
  
  # 3. Compute alpha by condition (AI vs User), also with key check
  alpha_by_condition <- data %>%
    group_by(Condition) %>%
    group_map(~ psych::alpha(.x[columns], check.keys = FALSE), .keep = TRUE)
  
  # 4. Print results
  cat("overall Cronbach's alpha:\n")
  print(overall_alpha$total$raw_alpha)
  
  cat("\n cronbach's alpha by condition:\n")
  cat("AI:\n")
  print(alpha_by_condition[[1]]$total$raw_alpha)
  cat("User:\n")
  print(alpha_by_condition[[2]]$total$raw_alpha)

  alpha_text <- paste0("Overall Cronbach's α = ", round(overall_alpha$total$raw_alpha, 2),
                     " | AI = ", round(alpha_by_condition[[1]]$total$raw_alpha, 2),
                     " | User = ", round(alpha_by_condition[[2]]$total$raw_alpha, 2))

  p <- p + labs(caption = alpha_text)

  
  # Save plot
  ggsave(filename = paste0("plots/","Questionnaire_",title, ".png"), plot = p, width = 10, height = 6, dpi = 300, bg = "white")
  
  # 7. Split data into AI and User
  data_ai <- data %>% filter(Condition == "AI")
  data_user <- data %>% filter(Condition == "User")
  
  items_ai <- data_ai[columns]
  items_user <- data_user[columns]
  
  # 8. Compute correlation matrices by condition
  cor_ai <- round(cor(items_ai, use = "pairwise.complete.obs"), 2)
  cor_user <- round(cor(items_user, use = "pairwise.complete.obs"), 2)
  
  # 9. Print correlation tables (ready to copy into PowerPoint)
  cat("\n🔷 Correlation matrix – AI initiated:\n")
  print(kable(cor_ai, format = "markdown"))
  
  cat("\n🔶 Correlation matrix – User initiated:\n")
  print(kable(cor_user, format = "markdown"))
  
  # Optional: Export as CSV for use in Excel/PowerPoint
  write.csv(cor_ai, paste0(title,"correlation_matrix_AI.csv"))
  write.csv(cor_user,  paste0(title, "correlation_matrix_User.csv"))
...
# Call the function
Questionnaire_plot(data, questions = my.labels, columns = 13:18, title = "Average Perceived usefulness (PU) per question by Condition",title_offset=0)
```
...
# Call the function
Questionnaire_plot(data, questions = my.labels, columns = 20:25, title = "Average Perceived Ease of Use (PEU) per question by Condition",title_offset=-0.6)
...
# Call the function
Questionnaire_plot(data, questions = my.labels, columns = c(12,19,26,27), title = "Average Scores for each construct",title_offset = 0.4)
