...
def fakestream(url, audio_data, samplerate, instance_name):
    """
...
def push_audio_track_stream(url, audio_data, samplerate, instance_name):
    """
    This function pushes audio chunks sequentially via PushAudioStreamRequest().
    The function splits the input audio data into smaller chunks to avoid exceeding gRPC limits.
    """

    # Define chunk size (e.g., 1 second of audio data)
    chunk_size = samplerate  # 1 second of audio data
    block_until_playback_is_finished = True  # ADJUST
    ...
    with grpc.insecure_channel(url) as channel:
        ...
        def make_generator():
            # Send the start marker with metadata
            start_marker = audio2face_pb2.PushAudioRequestStart(
                samplerate=samplerate,
                instance_name=instance_name,
                block_until_playback_is_finished=block_until_playback_is_finished,
            ...
            # Split audio_data into chunks and send each chunk
        ...
        # Send the audio data stream
        request_generator = make_generator()
        print("Sending audio data in chunks...")
        response = stub.PushAudioStream(request_generator)
        if response.success:
            print("SUCCESS")
        else:
