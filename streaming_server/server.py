...
sys.modules["omni.audio2face"] = MagicMock()
sys.modules["omni.audio2face.common"] = MagicMock()
...
if (
    ...
    log_warn(
        "DEV MODE (turn it off in production): api_implementation.Type() == {}".format(
...
class Audio2FaceServicer(audio2face_pb2_grpc.Audio2FaceServicer):
    ...
    def PushAudio(self, request, context):
        ...
        if self._audio_end_callback is not None:
            try:
                self._audio_end_callback(
                    instance_name, block_until_playback_is_finished
                )
            except RuntimeError as e:
                return audio2face_pb2.PushAudioResponse(success=False, message=str(e))
    ...
    def PushAudioStream(self, request_iterator, context):
        ...
        if not first_item.HasField("start_marker"):
            return audio2face_pb2.PushAudioResponse(
                success=False,
                message="First item in the request should containt start_marker",
            )
        ...
        block_until_playback_is_finished = (
            first_item.start_marker.block_until_playback_is_finished
        )
        log_info(
            "PushAudioStream request: [instance_name = {} ; samplerate = {}]".format(
...
class StreamingServer:
    ...
    def start(self, audio_start_callback, push_chunk_callback, audio_end_callback):
        ...
        audio2face_pb2_grpc.add_Audio2FaceServicer_to_server(
            Audio2FaceServicer(
                audio_start_callback, push_chunk_callback, audio_end_callback
            ),
            self._server,
        )
        success = _server._add_insecure_port(
            self._server._state, _common.encode(self._address)
        )
        while success == 0:
